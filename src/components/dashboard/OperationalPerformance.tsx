...
export const OperationalPerformance: React.FC = () => {
        ...
        Object.entries(trip.executions).forEach(([key, exec]: [string, any]) => {
          ...
          if (step.cargoItems?.[itemIdx]) {
            ...
            const goodsList = getSpecificGoodsList(
              ...
              stopIdx,
              trip.id,
              trip.routeProgress?.steps?.length || 1
            );
  ...
  return (
    <div className="bg-white dark:bg-[#121212] rounded-[24px] border border-gray-100 dark:border-[#2d2d2d] p-4 flex flex-col gap-4">
      <div className="flex items-center justify-between shrink-0">
        ...
        <button 
          onClick={refresh}
          className="w-8 h-8 flex items-center justify-center bg-white dark:bg-[#1a1c1a] border border-gray-200 dark:border-[#2d2d2d] rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors shadow-sm cursor-pointer"
        >
              ...
              {m.chartType === 'line' ? (
                <div className={`h-16 ${m.color} -mx-5 -mb-5 mt-0`}>
                  <svg viewBox="0 0 100 40" className="w-full h-full" preserveAspectRatio="none">
                    <path d={m.chartPath} fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                    <path d={`${m.chartPath} L100,40 L0,40 Z`} className={m.chartBg} fill="currentColor" />
                  </svg>
                </div>
              ) : (
